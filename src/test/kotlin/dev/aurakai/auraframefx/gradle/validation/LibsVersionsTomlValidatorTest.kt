...
class LibsVersionsTomlValidatorTest {
    ...
    @Test
    fun `ValidationResult addError should modify state correctly`() {
        val result = ValidationResult()
    ...
    fun `ValidationResult addWarning should not affect validity`() {
        ...
        assertTrue(result.isValid) // Should remain valid
        assertEquals(1, result.warnings.size)
        assertEquals("Test warning", result.warnings.first())
    }

    @Test
    fun `validate should handle TOML with empty sections`() {
        val emptyVersionsToml = """
            [versions]
        ...
        testFile.writeText(emptyVersionsToml)
    ...
    @Test
    fun `validate should handle TOML with invalid section headers`() {
        val invalidSectionToml = """
            [versions
            test = "1.0.0"
        ...
        testFile.writeText(invalidSectionToml)
    ...
    @Test
    fun `validate should handle modules with group and name format`() {
        val groupNameToml = """
            [versions]
            test = "1.0.0"
            
            [libraries]
            lib-with-group = { group = "com.example", name = "library", version.ref = "test" }
            lib-with-module = { module = "com.example:library", version.ref = "test" }
        """.trimIndent()
        
        testFile.writeText(groupNameToml)
    ...
    fun `validate should detect missing module property in libraries`() {
        ...
        # Should be valid as group/name format might be acceptable
        assertTrue(result.isValid || result.errors.any { it.contains("module") })
    }
    ...
    fun `validate should handle plugins without version references`() {
        val pluginDirectVersionToml = """
            ...
            plugin-with-ref = { id = "com.example.another", version.ref = "test" }
        """.trimIndent()
    ...
    @Test
    fun `validate should handle TOML with whitespace in values`() {
        val whitespaceToml = """
            [versions]
            padded = " 1.0.0 "
            normal = "1.0.0"
        ...
        # Should fail validation due to whitespace in version
        assertFalse(result.isValid)
        assertTrue(result.errors.any { it.contains("Invalid version format") })
    }

    @Test
    fun `validate should handle complex version compatibility scenarios`() {
        ...
        assertTrue(result.isValid)
        assertFalse(result.errors.any { it.contains("incompatibility") })
    }

    @Test
    fun `validate should detect AGP version incompatibility correctly`() {
        val incompatibleToml = """
            [versions]
            agp = "8.5.0"
            kotlin = "1.8.0"
        ...
        # Should detect incompatibility between AGP 8.x and Kotlin 1.8
        assertFalse(result.isValid)
        assertTrue(result.errors.any { it.contains("not compatible") })
    }

    @Test
    fun `validate should handle TOML with numeric keys properly`() {
        val numericKeysToml = """
            [versions]
            "123" = "1.0.0"
            "456-test" = "2.0.0"
    ...
    @Test
    fun `validate should handle vulnerable version detection for known libraries`() {
        val vulnerableToml = """
            [versions]
            junit = "4.10"
            safe-junit = "5.8.2"
            ...
            [libraries]
            junit-vulnerable = { module = "junit:junit", version.ref = "junit" }
            junit-safe = { module = "org.junit.jupiter:junit-jupiter", version.ref = "safe-junit" }
        """.trimIndent()
    ...
    @Test
    fun `validate should handle missing critical dependencies warning`() {
    ...
    @Test
    fun `validate should handle TOML with special characters in module names`() {
    ...
    fun `validate should handle large TOML files efficiently`() {
        ...
        # Add 1000 version entries to test performance
        for (i in 1..1000) {
    ...
    @Test
    fun `validate should handle TOML with empty bundle arrays`() {
        val emptyBundleToml = """
            [versions]
            test = "1.0.0"
    ...
    @Test
    fun `validate should handle bundle validation edge cases`() {
        val bundleEdgeCaseToml = """
            [versions]
            test = "1.0.0"
            ...
            valid = ["lib1", "lib2"]
            invalid = ["lib1", "nonexistent"]
            mixed = ["lib1", "lib2", "missing"]
        """.trimIndent()
        
        testFile.writeText(bundleEdgeCaseToml)
        
        val result = validator.validate()
        
        assertFalse(result.isValid)
        assertTrue(result.errors.any { it.contains("Invalid bundle reference") && it.contains("nonexistent") })
        assertTrue(result.errors.any { it.contains("Invalid bundle reference") && it.contains("missing") })
    }

    @Test
    fun `validate should handle TOML with different plugin ID formats`() {
        val pluginFormatsToml = """
            [versions]
            plugin-version = "1.0.0"
            ...
            kotlin-plugin = { id = "org.jetbrains.kotlin.jvm", version.ref = "plugin-version" }
            custom = { id = "my.custom.plugin", version.ref = "plugin-version" }
            invalid-short = { id = "invalid", version.ref = "plugin-version" }
            invalid-toolong = { id = "verylongpluginnamewithoutdots", version.ref = "plugin-version" }
        """.trimIndent()
        
        testFile.writeText(pluginFormatsToml)
        
        val result = validator.validate()
        
        assertFalse(result.isValid)
        assertTrue(result.errors.any { it.contains("Invalid plugin ID format") && it.contains("invalid") })
        assertTrue(result.errors.any { it.contains("Invalid plugin ID format") && it.contains("verylongpluginnamewithoutdots") })
    }

    @Test
    fun `validate should handle timestamp edge cases correctly`() {
    ...
    @Test
    fun `validate should handle file read errors gracefully`() {
        # Create a file in a directory that will be deleted
        val tempSubDir = tempDir.resolve("subdir").toFile()
        tempSubDir.mkdirs()
        val fileInSubDir = tempSubDir.resolve("test.toml")
        fileInSubDir.writeText("valid content")
        
        val validator = LibsVersionsTomlValidator(fileInSubDir)
        
        # Delete the parent directory to cause read error
    ...
    @Test
    fun `validate should handle TOML parsing errors for malformed content`() {
        val malformedToml = """
            [versions
            test = "1.0.0
            broken syntax here
    ...
    @Test
    fun `validate should handle TOML with mixed array and non-array bundle definitions`() {
        val mixedBundleToml = """
            [versions]
            test = "1.0.0"
        ...
        testFile.writeText(mixedBundleToml)
        
        val result = validator.validate()
        
        # Should handle non-array bundle definitions gracefully
        assertTrue(result.isValid || result.errors.any { it.contains("Syntax error") })
    }

    @Test
    fun `validate should handle TOML with extremely long version strings`() {
        val longVersion = "1.0.0-" + "a".repeat(10000)
        val longVersionToml = """
            [versions]
            long = "$longVersion"
        ...
        # Should handle extremely long strings gracefully
        assertTrue(result.isValid || result.errors.isNotEmpty())
    }

    @Test
    fun `ValidationResult should maintain proper state across multiple operations`() {
        val result = ValidationResult()
        ...
        assertTrue(result.isValid) # Still valid after warning
        assertEquals(1, result.warnings.size)
        
        result.addError("Error 1")
        assertFalse(result.isValid) # Now invalid
        assertEquals(1, result.errors.size)
        assertEquals(1, result.warnings.size)
    ...
    @Test
    fun `validate should handle TOML with non-standard but valid version patterns`() {
        val nonStandardToml = """
            [versions]
            single = "1"
            double = "1.2"
            ...
            [libraries]
            lib1 = { module = "group:artifact", version.ref = "triple" }
        """.trimIndent()
        ...
        # Most should be valid according to the regex pattern
        assertTrue(result.isValid)
        assertTrue(result.errors.isEmpty())
    }
    ...
    fun `validate should handle concurrent file modifications during validation`() {
        ...
        # Modify file during validation (simulate concurrent access)
        testFile.writeText("[versions]\ninvalid = \"broken\"\n")
        
        # Validate again
        val result2 = validator.validate()
        
        # Results should be consistent with their respective file states
        assertTrue(result1.isValid)
        assertFalse(result2.isValid)
    }

    @Test
    fun `validate should handle TOML with Unicode characters in various contexts`() {
        val unicodeToml = """
            [versions]
            "测试版本" = "1.0.0"
            "版本-español" = "2.0.0"
            ...
            [libraries]
            unicode-lib = { module = "group:artifact", version.ref = "测试版本" }
        """.trimIndent()
