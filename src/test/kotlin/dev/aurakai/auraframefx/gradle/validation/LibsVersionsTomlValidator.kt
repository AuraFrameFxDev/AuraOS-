...
class LibsVersionsTomlValidator(private val tomlFile: File) {
    ...
    /**
     * Performs all structural and content validations on the provided Gradle `libs.versions.toml` file content.
     *
     * Runs checks for required sections, version and module formats, duplicate keys, reference integrity, critical dependencies, version compatibility, bundle validity, and known security vulnerabilities. Appends any detected errors or warnings to the supplied lists.
     *
     ...
     *
     * Adds an error to the provided list if either section is missing.
     */
     ...
     * @param warnings Unused in this method; included for interface consistency.
     */
    ...
    /**
     * Checks that all version references in the TOML content correspond to defined versions and warns about defined versions that are not referenced.
     *
     ...
     *
     * Adds an error for each module entry that does not conform to the required format.
     *
     ...
     *
     * Adds an error for each plugin ID that does not match the expected pattern.
     */
    ...
    /**
     * Issues a warning if the TOML content does not include any critical testing dependencies.
     *
    ...
    /**
     * Checks for incompatible version combinations in the TOML content.
     *
     * Adds an error if both AGP version 8.11.1 and Kotlin version 1.8.0 are present, indicating that this combination is not supported.
     */
     ...
     *
     * @param content The TOML file content as a string.
     * @param errors A mutable list to which error messages are added for invalid bundle references.
     */
