...
class LibsVersionsTomlValidator(private val tomlFile: File) {
    ...
    /**
     * Performs comprehensive validation of a Gradle version catalog TOML file.
     *
     ...
     *
     * Recognizes section headers, key-value pairs, inline tables, and arrays, while ignoring comments and empty lines. Throws a runtime exception if invalid TOML syntax is encountered.
     *
     ...
     *
     * Converts the input string to a quoted string, inline table (as a map), or array (as a list), based on TOML syntax. Returns the raw string if the value does not match any recognized TOML structure.
     *
    ...
    /**
     * Converts a TOML array string into a list of string elements.
     *
     * The input should be a TOML array (e.g., `["item1", "item2"]`). Each element is trimmed and stripped of surrounding double quotes.
     *
     ...
     *
     * For each section, if a key appears more than once, an error message is added to the validation result.
     */
    ...
    /**
     * Checks that all version references in the "libraries" and "plugins" sections exist in the "versions" section, and issues warnings for unused versions.
     *
    ...
    /**
     * Validates that each library's module string matches the required module pattern.
     *
     * Records an error for every library whose module string does not conform to the expected format.
     */
     ...
     *
     * Records an error for each plugin whose "id" does not match the expected format.
     */
     ...
     *
     * Records an error in the validation result if this specific version pairing is found.
     */
    ...
    /**
     * Issues warnings for libraries that use versions listed as vulnerable.
     *
     * For each library, checks its resolved version (either direct or via version reference) against the `VULNERABLE_VERSIONS` map. Adds a warning to the validation result if a vulnerable version is detected.
     */
    ...
    /**
     * Checks that all bundle references in the TOML data point to libraries defined in the "libraries" section.
     *
     * Records an error for each bundle that references a non-existent library.
     */
    ...
    /**
     * Issues a warning if critical dependencies are not present in the libraries section.
     *
     * Checks for the presence of essential dependencies such as "junit:junit" and "androidx.core:core-ktx" among the defined library modules. Adds a warning to the validation result if any are missing.
     */
