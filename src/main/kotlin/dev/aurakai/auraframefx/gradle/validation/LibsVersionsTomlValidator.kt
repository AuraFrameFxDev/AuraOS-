...
class LibsVersionsTomlValidator(private val tomlFile: File) {
    ...
    /**
     * Parses TOML file content into a map of section names to their key-value pairs.
     *
     ...
     *
     * Handles quoted strings, inline tables, arrays, and unquoted raw strings, returning the appropriate type.
     *
     * @param value The TOML value as a string.
     * @return The parsed value as a String, Map, or List, depending on the TOML format.
     */
    ...
    /**
     * Parses a TOML inline table string (enclosed in curly braces) into a map of key-value pairs.
     *
     * The input must be a valid TOML inline table, with comma-separated key-value pairs (e.g., `{ key1 = "value1", key2 = "value2" }`).
     *
     ...
     *
     * The input must be a TOML array (e.g., `["item1", "item2"]`). Returns the individual string elements as a list.
     *
     * @param value TOML array string to convert.
     * @return List of strings contained in the array.
     */
     ...
     *
     * Records an error in the validation result for every duplicate key found within a section.
     *
     * @param tomlData Parsed TOML data organized by section.
     * @param result The validation result to which errors are appended.
     */
    ...
    /**
     * Validates plugin IDs in the "plugins" section of the TOML data.
     *
     ...
     *
     * Warns when a library's version matches any entry in the `VULNERABLE_VERSIONS` map.
     */
     ...
     *
     * Warns if critical modules such as "junit:junit" or "androidx.core:core-ktx" are not defined in the version catalog.
     */
